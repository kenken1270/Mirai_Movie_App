...
def page_dashboard_pc(supabase) -> None:
    ...
    c1.metric("💡 アイデア数", total_themes)
    c2.metric("📝 台本数", total_scripts)
    c3.metric("🎬 プロジェクト数", total_projects)
    ...
    with col_left:
        ...
        if themes:
            ...
            df_chart = pd.DataFrame(
                {"ステータス": list(status_counts.keys()), "件数": list(status_counts.values())}
            )
    ...
    for proj in project_list:
        ...
        with st.expander(f"{emoji} {title}  [{platform}]  ― {status}"):
            ...
            with col_edit:
                ...
                new_platform = st.selectbox("プラットフォーム", ["YouTube", "TikTok", "Instagram", "その他"],
                                            index=["YouTube","TikTok","Instagram","その他"].index(platform) if platform in ["YouTube","TikTok","Instagram","その他"] else 0,
                                            key=f"platform_{pid}")
                new_status   = st.selectbox("ステータス", ["撮影中", "投稿前（撮影済み）", "投稿済み"],
                                            index=["撮影中","投稿前（撮影済み）","投稿済み"].index(status) if status in ["撮影中","投稿前（撮影済み）","投稿済み"] else 0,
                                            key=f"status_{pid}")
                new_date     = st.date_input("投稿予定日", key=f"date_{pid}",
            ...
            with col_metric:
                ...
                st.markdown("**📈 推移グラフ**")
                try:
                    metrics_data = supabase.table("project_metrics").select("*")\
                        .eq("project_id", pid).order("recorded_date").execute().data or []
                    if metrics_data:
                        df_m = pd.DataFrame(metrics_data)
                        df_m = df_m[["recorded_date", "views", "likes", "comments"]].rename(
                            columns={"recorded_date": "日付", "views": "視聴回数", "likes": "いいね", "comments": "コメント"}
                        )
                        df_m = df_m.set_index("日付")
                        st.line_chart(df_m)
                    else:
                        st.info("まだ記録がありません。")
                except Exception as e:
